...
| C3 | **Tax / fee engine** | Per-location tax rates (inclusive/exclusive), delivery distance fees, service fees, tip base rules, rounding policy (per-line vs per-total, documented). Pro later: VAT classes, multi-rate | Keep V1 to one rate per location + one fee stack; extensible via `smooth_restaurant_totals_adjustments` filter |
