...
final class Plugin {
	...
	private function registerProviders(): void {
		...
		if ( ! is_array( $filtered ) ) {
			$this->logSkipped( 'smooth_service_providers', 'filter-must-return-array: falling back to the Free provider list.' );
			$filtered = $free;
		}
	...
	private function isValidProvider( mixed $candidate, bool $alreadyLogged = false ): bool {
		if ( ! is_string( $candidate ) ) {
			if ( ! $alreadyLogged ) {
				$this->logSkipped( '(non-string:' . gettype( $candidate ) . ')', 'unknown-class: provider entries must be class-strings.' );
			}
		...
		if ( ! is_string( $version ) || version_compare( $version, self::PROVIDER_VERSION_FLOOR, '<' ) ) {
			$this->logSkipped( $candidate, 'version-floor: provider version is below ' . self::PROVIDER_VERSION_FLOOR . '.' );
